...
namespace Elmah.WebArchiver
{
    ...
    public class ErrorLogArchiveHandler : HttpTaskAsyncHandler
    {
        ...
        async void ProcessRequestAsync2(HttpContext context)
        {
            var log = GetErrorLog(context);
            var pages = log.PageAllErrors(100,
                                          t => new { Task = t,           Entries = (ErrorLogEntry[]) null },
                                          e => new { Task = (Task) null, Entries = e                      });
            ...
            {
                // ReSharper disable once AccessToDisposedClosure
                await Archive(pages, p => p.Task, p => p.Entries, Encoding.UTF8, 
                              e => zip.CreateEntry(string.Format("error-{0}", e.Id)).Open());
            }
        }

        static async Task Archive<T>(IEnumerable<T> pages, 
            Func<T, Task> taskSelector, 
            Func<T, IEnumerable<ErrorLogEntry>> entriesSelector, 
            Encoding encoding, 
            Func<ErrorLogEntry, Stream> opener)
        {
            ...
            foreach (var page in from page in pages
                                 select new
                                 {
                                     Task    = taskSelector(page),
                                     Entries = entriesSelector(page),
                                 })
            {
                if (page.Task != null)
                {
                ...
                foreach (var e in page.Entries)
                {
                    using (var entryStream = opener(e))
                    {
                        var bytes = encoding.GetBytes(ErrorXml.EncodeString(e.Error));
