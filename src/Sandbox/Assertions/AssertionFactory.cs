...
[assembly: Elmah.Scc("$Id: AssertionFactory.cs 566 2009-05-11 10:37:10Z azizatif $")]
...
namespace Elmah.Assertions
{
    ...
    public sealed class AssertionFactory
    {
        ...
        {
            int delay = 0;
            XmlAttribute attribute = config.GetAttributeNode("delay");
            if (attribute != null)
            {
                delay = int.Parse(attribute.Value);
            }
            return new ThrottleAssertion(delay);
        }
