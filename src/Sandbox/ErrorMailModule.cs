...
[assembly: Elmah.Scc("$Id: ErrorMailModule.cs 725 2010-10-03 19:15:37Z azizatif $")]
...
namespace Elmah
{
    ...
    public class ErrorMailModule : HttpModuleBase, IExceptionFiltering
    {
...
#endif
        private int _throttleMailDelay;

        private Error _lastErrorMailed;
        ...
        protected override void OnInit(HttpApplication application)
        {
...
#endif
            int throttleMailDelay = Convert.ToUInt16(GetSetting(config, "throttleMailDelay", "0"));
            ...
            _useSsl = useSsl;
#endif
            _throttleMailDelay = throttleMailDelay;
        }
        ...
        /// <summary>
        /// Gets the email throttle delay in minutes
        /// </summary>
        protected int ThrottleMailDelay
        {
            get { return _throttleMailDelay;  }
        }
        ...
        protected virtual void OnError(Exception e, HttpContext context)
        {
            ...
            if (_throttleMailDelay > 0)
            {
                //check to see if if the same error was mailed within the set period of time.
                if (_lastErrorMailed != null)
                {
                    //check against the last exception to be emailed
                    if (CheckForDuplicateErrorMessage(error, _lastErrorMailed))
                    {
                        return;
                    }
                }
                ...
                {
                    //check the error log to see if the current error matches 
                    //the previous logged error if the last error mailed is unknown
                    ErrorLog log = ErrorLog.GetDefault(context);

                    List<ErrorLogEntry> errorList = new List<ErrorLogEntry>();
                    int count = log.GetErrors(0, 10, errorList);

                    if (count > 0)
                    {
                        if (CheckForDuplicateErrorMessage(error, errorList[0].Error))
                        {
                            return;
                        }
                    }
                ...
                //persist the last error to be emailed
                _lastErrorMailed = error;
            }
        ...
        private bool CheckForDuplicateErrorMessage(Error currentError, Error previousError)
        {
            return previousError.Message == currentError.Message
                        && DateTime.Now.Subtract(previousError.Time).TotalMinutes < _throttleMailDelay;
        }
